...
`default_spaces_sidebar_token` optionally names one workspace metadata token
that the plugin wants shown with Herdr's default Spaces sidebar layout. The
plugin reports the value normally, for example with
`herdr workspace report-metadata ... --token github_pr=...`; Herdr places it
after branch and Git status. On an indented worktree, where those Git details
are hidden, the value uses its own row. Missing values, disabled plugins, and
uninstalled plugins, and plugins unsupported on this host add nothing.
Contributions are ordered by token name and duplicates are removed.

This default applies only while `[ui.sidebar.spaces].rows` is omitted. An
explicit row layout remains authoritative, including one equal to Herdr's
built-in rows; add `$github_pr` to that layout when you want the token there.
