...
自動接続と再接続は非対話型です。`! auth` の復旧は[SSH 認証の復旧](/ja/docs/connecting-machines/#ssh-認証の復旧)を参照してください。折りたたんだサイドバーでは `! auth` と `! attention` は `!` と表示されます。`! attention` の場合は、`herdr --remote workbox` など Herdr が表示する単独接続用コマンドをフォアグラウンドで実行し、クライアントを再起動してください。名前付きセッションを選んだプロファイルにだけ `--session <name>` を追加します。
