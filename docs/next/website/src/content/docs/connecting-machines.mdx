...
On Linux and macOS clients, when `remote.manage_ssh_config=true`, Herdr can share an approved OpenSSH connection across machine add, the sidebar, and saved-machine API commands. Click the **`! auth`** badge on the affected machine—not the machine group row—to open a client-private terminal popup. It uses native OpenSSH prompts; Esc or Cancel aborts. Herdr stores no OTP and closes the popup only after the saved endpoint completes a verified handshake. The popup does not install or update Herdr. The non-authentication state is **`! attention`**; in a collapsed sidebar, either state appears as `!`.
...
- **Attention:** The target needs an action that cannot be completed in the background, such as host-key approval or a compatible server. Other machines remain usable. **`! auth`** means authentication failed; click that badge to start recovery. **`! attention`** indicates another issue. In a collapsed sidebar, either appears as `!`.
...
Use your profile's target. If you chose a named session when adding it, include the optional `--session <name>` here too. Follow the requested setup prompts, then restart the client to retry the `! attention` connection. The authentication popup does not install or update Herdr. Do not stop a running server merely because its version differs from the client.
