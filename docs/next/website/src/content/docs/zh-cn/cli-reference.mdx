...
自动连接和重连均为非交互式。`! auth` 的恢复请参阅[恢复 SSH 认证](/zh-cn/docs/connecting-machines/#恢复-ssh-认证)；侧边栏折叠时，`! auth` 和 `! attention` 都显示为 `!`。对于 `! attention`，请在前台运行 Herdr 显示的独立连接命令，例如 `herdr --remote workbox`，然后重启客户端。仅当配置指向命名会话时才添加 `--session <name>`。
